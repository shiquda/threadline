...
type Page = "overview" | "inbox" | "workboard" | "decisions" | "initiative" | "decision";
...
function MiniInboxItem({ item }: { item: InboxItem }) { return <a className="mini-inbox" href={item.decision ? `#decision/${item.decision.id}` : "#inbox"}><Badge tone={item.decision ? "decision" : item.submission.kind}>{titleCase(item.decision ? "decision" : item.submission.kind)}</Badge><div><strong>{item.submission.title}</strong><span>{item.submission.runtime ?? item.submission.source} · {formatDate(item.submission.created_at)}</span></div><ChevronRight size={17} /></a>; }
...
function SubmissionRow({ submission }: { submission: Submission }) { return <div className="linked-row"><div><strong>{submission.title}</strong><span>{submission.summary}</span></div><Badge tone={submission.kind}>{titleCase(submission.kind)}</Badge></div>; }
