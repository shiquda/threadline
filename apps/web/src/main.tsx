...
function InboxPage({ api }: { api: ThreadlineApi }) {
  ...
  return <PageHeader title={t("Inbox")} subtitle={scope === "unread" ? t("Only things that need your attention right now.") : t("All inbox messages, including those already read.")} action={<button className="btn btn-secondary btn-sm" disabled={!unread.length} onClick={() => Promise.all(unread.map((item) => api.updateNotification(item.notification.id, "read"))).then(data.reload)}>{t("Mark all read")}</button>}>
